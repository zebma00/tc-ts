...
const ItemDmg: React.FC<{ damage: string | null | undefined; speed: string | null; dps: string | null }> = ({
	damage,
	speed,
	dps,
}) => {
...
const SingleItem: React.FC<Item> = ({
	name,
	img,
	quality,
	slot,
			...
			<div
				...
				{playerClass && (
					<div className={styles.singleItemPlayerClass}>
						Classes: <span style={{ color: getClassColor(playerClass.toLowerCase()) }}>{playerClass}</span>
					</div>
				)}
				<div className={styles.itemDescription}>
					{description && description.map((line, i) => <div key={i}>{line}</div>)}
				</div>
