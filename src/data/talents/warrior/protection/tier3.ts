import { Talent } from '../../Classes'

export default [
  new Talent(
    'Last Stand',
    'reflection',
    ['When activated, temporarily increases maximum health by 30% for 20 sec. After the effect expires, the hit points are lost.'],
    1,
    [[]],
    [{ x: 1, y: 0 }],
    null,
    null,
    null,
    'Instant',
    '10 min cooldown'
  ),
  new Talent(
    'Improved Shield Block',
    'reflection',
    [
      'Allows your Shield Block to block an additional attack and increases the duration by ',
      ' seconds. In addition, when you block an attack, you generate additional threat.',
    ],
    2,
    [[0.5, 1, 2]],
    [{ x: 0, y: 1 }]
  ),
  new Talent('Improved Revenge', 'insect-swarm', ['Gives your Revenge a ', '% chance to stun the target for 3 sec.'], 3, [[15, 30, 45]]),
  new Talent('Defiance', 'sublety', ['Increases the threat generated by your attacks while in defensive stance by ', '%.'], 3, [[5, 10, 15]]),
]
